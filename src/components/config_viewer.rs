...
pub fn ConfigViewer(props: ConfigViewerProps) -> Element {
    ...
    let active_class = "flex items-center gap-2 px-6 py-2.5 text-sm font-bold rounded-xl transition-all bg-white text-indigo-600 shadow-sm";
    ...
    let editor_active = "bg-indigo-600/20 text-indigo-400 border-indigo-500/30";
    let editor_inactive =
            ...
            div { class: "w-full max-w-3xl overflow-hidden rounded-[2.5rem] bg-zinc-950 border border-zinc-800 shadow-2xl animate-scale-in",
                ...
                div { class: "flex items-center justify-between border-b border-zinc-900 p-8",
                    div {
                        ...
                        p { class: "text-sm text-zinc-400", "Choose how you want to integrate with your editor." }
                    }
                    button {
                ...
                div { class: "p-8 space-y-8",
                    ...
                    div { class: "flex flex-col items-center gap-6",
                        ...
                        div { class: "flex flex-wrap justify-center gap-2",
                            {
                                [TargetEditor::Claude, TargetEditor::Cursor, TargetEditor::Windsurf, TargetEditor::OpenCode, TargetEditor::Antigravity].into_iter().map(|target| {
                                    let is_active = *editor.read() == target;
                                    let current_class = if is_active { editor_active } else { editor_inactive };
                    ...
                    // Info Box
                    div { class: "flex items-start gap-4 p-4 rounded-2xl bg-indigo-500/5 border border-indigo-500/10",
                    ...
                    div { class: "grid grid-cols-2 gap-4",
                        div { class: "p-5 rounded-3xl bg-zinc-900/50 border border-zinc-900",
                            h4 { class: "text-xs font-bold uppercase tracking-widest text-zinc-500 mb-3", "macOS Location" }
                            code { class: "text-[11px] font-mono text-zinc-300 break-all leading-relaxed",
                                "{editor.read().macos_path()}"
                            }
                        }
                        div { class: "p-5 rounded-3xl bg-zinc-900/50 border border-zinc-900",
                            h4 { class: "text-xs font-bold uppercase tracking-widest text-zinc-500 mb-3", "Windows Location" }
                            code { class: "text-[11px] font-mono text-zinc-300 break-all leading-relaxed",
